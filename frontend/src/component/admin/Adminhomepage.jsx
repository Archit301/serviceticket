  ...
  const [soldoutTicket,setsoldoutTicket]=useState("")
    useEffect(() => {
        const fetchStats = async () => {
            try {
                const res = await fetch(`/backend/ticket/countticket/${currentUser._id}`);
    ...
    useEffect(() => {
        ...
        const fetchStats = async () => {
            try {
                const res = await fetch(`/backend/ticket/countsoldoutticket/${currentUser._id}`);
               const data= await res.json();
               console.log(data);
    ...
    useEffect(() => {
      ...
      //  console.log(currentUser);
        const fetchStats = async () => {
            try {
                const res = await fetch(`/backend/ticket/countavailableticket/${currentUser._id}`);
               const data= await res.json();
               console.log(data);
    ...
    useEffect(() => {
      ...
      //  console.log(currentUser);
        const fetchStats = async () => {
            try {
                const res = await fetch(`/backend/ticket/countexpiryticket/${currentUser._id}`);
               const data= await res.json();
               console.log(data);
    ...
    const handleSeeall=()=>{
     navigate('/adminticketname');
    }

    const handleSeeavailable=()=>{
        navigate('/adminticketavailablename');
       }

       const handleSeesoldout=()=>{
        navigate('/adminticketsoldoutname');
       }

       const handleSeeexpiry=()=>{
        navigate('/adminticketexpiryname');
       }
