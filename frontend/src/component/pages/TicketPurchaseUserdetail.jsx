...
const TicketPurchaseUserdetail = () => {
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto bg-white shadow-md rounded-lg overflow-x-auto">
      ...
      <form onSubmit={handleSubmit} className=" flex  p-4 bg-white shadow rounded-md">
      ...
      <table className="w-full bg-white divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
          <th className="px-8 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">S.No</th>
            <th className="px-8 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Image</th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Username</th>
            <th className="px-12 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Email</th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date of Purchase</th>
            { currentUser.role==='admin' &&(
            <th className="px-8 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Feedback</th>
            )
}
          </tr>
        </thead>
          ...
          {purchases.length > 0 ? (
            purchases.map((purchase,index) => (
              <tr key={index}>
                ...
                <td className="px-8 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{purchase.username}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{purchase.email}</td>
                <td className="px-10 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(purchase.createdAt).toLocaleDateString()}</td>
                { currentUser.role==='admin' &&(
            ...
            <tr>
              <td colSpan="4" className="px-6 py-4 text-center text-gray-500">No purchase details available</td>
            </tr>
          )}
        </tbody>
      </table>
            ...
            {pageNumbers.map(number => (
              <li key={number}>
                <button
                  ...
                  className={`px-3 py-1 text-sm font-medium rounded-md ${currentPage === number ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'}`}
                >
                  {number}
                </button>
              </li>
